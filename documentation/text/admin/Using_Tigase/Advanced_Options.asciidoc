...
Tigase can now support users with empty nicknames.  This can be enabled by adding the following code in init.tdsl.
[source,dsl]
------
'sess-man' {
    'jabber:iq:roster' {
...
To enable this to be added to the counter, the following line must be in your init.tdsl file.
[source,dsl]
-----
c2s {
...
-----
The above setting in init.tdsl file will change the default timeout period to 90 seconds.
