...
Default virtual hosts property is now able to be configured only as a domain name instead of the list of virtual host domains with options. Additional virtual host domains need and their options need to be configured using ad-hoc commands.  Reference xref:virtHosts[Virtual-Hosts Configuration] for more details.
