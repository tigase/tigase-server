...
In order to have more freedom while accessing repositories it's possible to use:

* *AuthRepository*
* *UserRepository*
* custom repository implementation which implements *DataSourceAware* interface
...
As mentioned it is also possible to use custom repository class. For our example let's assume it will be class implementing *TestRepositoryIfc* and our implementation will be using JDBC.
