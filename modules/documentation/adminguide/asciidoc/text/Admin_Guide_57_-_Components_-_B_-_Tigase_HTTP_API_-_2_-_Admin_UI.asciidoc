...
Get list of available commands is one script, that is present for every component that is active on the server, and as its title imples, will provide a list of all commands for that component.  Lastly, the two scripts from the xref:scriptingSupport[Scripting section] of this guide.  Generally, there is not much needed to see in this section.
...
- Anonymous enabled: Allow anonyous logins.
...
- Domain filter domains: a spcecific setting to restrict or controll cross domain traffic.
- Max users: maximum users allowed to be registered to the server.
- Allowed C2S, BOSH, Websocket ports: Comma seperated list of ports that this vhost will check for all of these services.
...
- Message foward address: Specific address where all messages will be sent too.  This may be useful to you if you have a single server handling AMP or message storage and want to keep the load there.
...
- Administrators: comma seperated list of JIDs who will be considered admins for the vhost.
...
- XEP-0136 retention period: interget of number of days message archives will be set.
- Trusted JIDs: Comma seperated list of JIDs who will be added to the trusted list, these are JIDS that may conduct commands, edit settings, or other secure work without needed secure logins.
...
- XEP-0136 - store MUC messages: Whether or not to store MUC messages for archiving.  Default is user, which allows users to individually set this setting, otherwise tue.false will overrride.
- see-other-host redirection enabled: in servers that have mutiple clusters, this feature will help to automatically repopulate the cluster list if one goes down, however if this is unchecked, that list wil not change and may attempt to send traffic to a down server.
...
- Specify the subscriber model: Choose what type of subscriber model will be used for this node.  Options include:
  * *authorize* - Requires all subscriptions to be approved by the node owner before items will be published to the user.  Also only subscribers may reteieve items.
  ...
  * *whitelist* - Only explicitly allowed JIDs are allowed to subscribe and reteieve items from the node, this list is set by the owner/administrator.
...
- The domains allowed to access this node: Comma separated list of domains for which users can access this node.  By default is is blank, and there is no domain rescrition.
...
- The URL of the XSL transformation which can be applied to payloads in order to generate an appropriate message body element: ??????????????????
...
~~~~~~~~~~~~~~~~~~~~~~~~~
Typically two entries will be seen for this entry, one for basic-conf and another for vhost-man. They each have thier own sections, but provide for a plethoa of server options.  Changes to the server are done in realtime, and may not be permanent.
...
- Comma seperated of owner groups: Groups that the user wants to join/leave. *not required*
...
- Comma separated list of item gorups: A group or list of groups that the roster item JID will be added to/removed from.
- Action:
  * *Add/update item* - Will add or update the item JID in the roster owner's roster.
  * *Remove item* - Will remove the item JID from the roster owners's roster.
...
Scripts
-------
This section will enable administrators to custom write or enter thier own scripts for specific componenets.  Each active component will have an entry for new and remove command scripts and scripts written there will be for that component.

New Command Script
~~~~~~~~~~~~~~~~~~
- Description: A friendly name of the script, will be the title of the link in the menu on the left.
- Command ID: Internal command that tigase will use when referenceing this script.
- Group: The group for the script, which may be any of the headings on the left (Configuration, Example scrpts, Notifications, Other etc..) or your own.  If no group exists, a new one will be created.
...
You will be provided a list of scripts associated with that component.  You also have the open to remove from disk, which will permentally delete the script from the hard drive the server is on.  If this is unchecked, the script will be unavailable until the next restart.
