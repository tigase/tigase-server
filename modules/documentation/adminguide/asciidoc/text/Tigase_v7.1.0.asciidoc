...
- link:https://projects.tigase.org/issues/4760[#4760] Fixed NPE upon anonymous user conection, changed log notification to reduce clutter.

- #4762 Fixed delayed processing of WebSocket data frame if frame is both TCP and ping frame.

- link:https://projects.tigase.org/issues/4786[#4786] Changed timeout for cluster repository connection during acceptence of incoming cluster connection from 15ms to 15s.

- link:https://projects.tigase.org/issues/4787[#4787] Fixed possible NPE in ClientStateIndication when GETFEATURES command is executed before session is confirmed not null.

- link:https://projects.tigase.org/issues/4793[#4793] Fixed NPE in statistics occuring when certain values are null.

- link:https://projects.tigase.org/issues/4812[#4812] Fixed possible NPE in ClientStateIndication and mobile optimizations if packets are processed before resource binding is finished.

- link:https://projects.tigase.org/issues/4818[#4818] Fixed StampComparitor not comparing packets stamped with XEP-0203 Delayed Delivery instructions.

- link:https://projects.tigase.org/issues/4820[#4820] Now fixed issue where it was impossible to remove user from all groups.
