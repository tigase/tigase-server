...
Our preferred way to install the Tigase server is using xref:webinstall[Web installer] and configuration program which comes with one of the binary packages. Please pick up the latest version of the distribution archive in our link:https://tigase.net/downloads[download section].
