...
Inside the Tigase server link:https://github.com/tigase/tigase-server/blob/master/scripts/[scripts/] repository find a script called `machine-check.sh`. It performs all the basic checks from the article and also tries to adjust them when necessary. Have a link:https://github.com/tigase/tigase-server/blob/master/scripts/machine-check.sh[look at the code] and run for yourself.
