...
public class TigaseCustomAuth
	...
	public void initRepository(final String connection_str, Map<String, String> params)
		...
		try {
			...
			userscount_query = getParamWithDef(params, DEF_USERS_COUNT_KEY,
			...
			nonsasl_mechs = getParamWithDef(params, DEF_NONSASL_MECHS_KEY, DEF_NONSASL_MECHS)
					.split(",");
	...
	public void logout(BareJID user) throws UserNotFoundException, TigaseDBException {
		...
		try {
			PreparedStatement user_logout = data_repo.getPreparedStatement(user,
					userlogout_query);
	...
	public void updatePassword(BareJID user, final String password)
		...
		try {
			PreparedStatement update_pass = data_repo.getPreparedStatement(user,
	 ...
	 *
	 *  a <code>long</code> number of user accounts in database.
	 *
	...
	public long getUsersCount() {
		...
		try {
			long              users = -1;
			PreparedStatement users_count = data_repo.getPreparedStatement(null,
					userscount_query);
	...
	public long getUsersCount(String domain) {
		...
		try {
			long              users = -1;
			PreparedStatement users_domain_count = data_repo.getPreparedStatement(null,
	 ...
	 * @param def is a <code>String</code>
	 *
	 * @return a value of <code>String</code>
	 */
	...
	private boolean userLoginAuth(BareJID user, final String password)
		...
		try {
			PreparedStatement user_login = data_repo.getPreparedStatement(user,
					userlogin_query);
	...
	private String getPassword(BareJID user)
		...
		try {
			PreparedStatement get_pass = data_repo.getPreparedStatement(user,
					getpassword_query);
