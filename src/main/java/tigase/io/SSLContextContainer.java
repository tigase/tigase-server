...
public class SSLContextContainer
	...
	private ArrayList<X509Certificate> acceptedIssuers = new ArrayList<X509Certificate>(
			200);
	...
	// ~--- methods
	// --------------------------------------------------------------
	private Map<String, SSLContext> sslContexts = new ConcurrentSkipListMap<String,
			SSLContext>();
	...
	public void addCertificates(Map<String, String> params)
		...
		boolean saveToDisk = (saveToDiskVal != null) && saveToDiskVal.equalsIgnoreCase(
				"true");
		...
		if (pemCert != null) {
			try {
				CertificateEntry entry = CertificateUtil.parseCertificate(new CharArrayReader(
						pemCert.toCharArray()));
	 ...
	 *
	 *
	 * @param data is a <code>Map<String,T></code>
			...
			for (final Entry<String, File> entry : predefined.entrySet()) {
				try {
					...
					log.log(Level.CONFIG,
							"Loaded server certificate for domain: {0} from file: {1}", new Object[] {
							alias,
							entry.getValue() });
				} catch (Exception ex) {
			...
			for (String pemDir : pemDirs) {
				...
				for (File file : certsDirs[certsDirsIdx].listFiles(new PEMFileFilter())) {
					try {
						...
						addCertificateEntry(certEntry, alias, false);
						log.log(Level.CONFIG,
								"Loaded server certificate for domain: {0} from file: {1}",
								new Object[] { alias,
								file });
					} catch (Exception ex) {
	 ...
	 * @param hostname
	 * @param clientMode is a <code>boolean</code>
	 * @param tms is a <code>TrustManager</code>
	 *
	 *
	 *
	 * @return a value of <code>SSLContext</code>
		...
		try {
			...
			if (sslContext == null) {
				...
				if (kmf == null) {
					...
					X509Certificate cert = CertificateUtil.createSelfSignedCertificate(email,
							alias, ou, o, null, null, null, keyPair);
	...
	private KeyManagerFactory addCertificateEntry(CertificateEntry entry, String alias,
		...
		keys.setKeyEntry(alias, entry.getPrivateKey(), emptyPass, CertificateUtil.sort(entry
				.getCertChain()));
	...
	private void loadTrustedCerts(String[] trustLocations) {
		...
		try {
			...
			log.log(Level.CONFIG, "Loading trustKeyStore from locations: {0}", Arrays.toString(
					trustLocations));
		...
		try {
			if (!trustKeyStore.aliases().hasMoreElements()) {
				log.log(Level.CONFIG,
						"No Trusted Anchors!!! Creating temporary trusted CA cert!");

				KeyPair         keyPair = CertificateUtil.createKeyPair(1024, "secret");
				X509Certificate cert = CertificateUtil.createSelfSignedCertificate(
						"fake_local@tigase", "fake one", "none", "none", "none", "none", "US",
						keyPair);
		...
		tms = new X509TrustManager[] { new FakeTrustManager(acceptedIssuers.toArray(
				new X509Certificate[acceptedIssuers.size()])) };
	...
	private static class FakeTrustManager
		 ...
		 *
		 *
		 * @return a value of <code>X509Certificate[]</code>
		 */
		@Override
	...
	private class PEMFileFilter
		...
		public boolean accept(File pathname) {
			if (pathname.isFile() && (pathname.getName().endsWith(".pem") || pathname.getName()
					.endsWith(".PEM") || pathname.getName().endsWith(".crt") || pathname.getName()
					.endsWith(".CRT") || pathname.getName().endsWith(".cer") || pathname.getName()
					.endsWith(".CER"))) {
				return true;
			}
