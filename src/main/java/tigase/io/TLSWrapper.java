...
public class TLSWrapper {
	...
	private static final boolean tls_jdk_nss_workaround = Boolean.getBoolean(
	...
	private static final String[] tls_enabled_protocols = (System.getProperty(
			"tls-enabled-protocols") != null)
			? System.getProperty("tls-enabled-protocols").split(",")
			: null;
	 ...
	 */
	public TLSWrapper(SSLContext sslc, TLSEventHandler eventHandler, String[] sslProtocols,
			boolean clientMode, boolean wantClientAuth) {
	...
	public ByteBuffer unwrap(ByteBuffer net, ByteBuffer app) throws SSLException {
		...
		if (tlsEngineResult.getHandshakeStatus() == SSLEngineResult.HandshakeStatus
				.FINISHED) {
		...
		if (tlsEngineResult.getHandshakeStatus() == HandshakeStatus.NEED_TASK) {
			doTasks();
			if (log.isLoggable(Level.FINEST)) {
				log.log(Level.FINEST, "unwrap() doTasks(), handshake: {0}, {1}", new Object[] {
	...
	public void wrap(ByteBuffer app, ByteBuffer net) throws SSLException {
		...
		if (tlsEngineResult.getHandshakeStatus() == HandshakeStatus.NEED_TASK) {
			doTasks();
			if (log.isLoggable(Level.FINEST)) {
				log.log(Level.FINEST, "doTasks(): {0}, {1}", new Object[] { tlsEngine
						.getHandshakeStatus(),
						debugId });
			}
		}
	 ...
	 *
	 *
	 * @return a value of <code>CertCheckResult</code>
	 */
	 ...
	 *
	 *
	 * @return a value of <code>TLSStatus</code>
	 */
	public TLSStatus getStatus() {
		TLSStatus status = null;

		if ((tlsEngineResult != null) && (tlsEngineResult.getStatus() == Status
				.BUFFER_UNDERFLOW)) {
...
//    status = TLSStatus.NEED_READ;
	 ...
	 *
	 *
	 * @return a value of <code>SSLEngine</code>
	 */
...
//      ByteBuffer bb = ByteBuffer.allocate(app.capacity() + appBuffSize);
		if (log.isLoggable(Level.FINE)) {
			log.log(Level.FINE, "Resizing tlsInput to {0} bytes, {1}", new Object[] { (2048 +
					app.capacity()),
					debugId });
		}
