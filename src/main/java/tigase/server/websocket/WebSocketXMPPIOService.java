...
public class WebSocketXMPPIOService<RefObject>
	...
	private static final Logger log = Logger.getLogger(WebSocketXMPPIOService.class
			.getCanonicalName());
	...
	private void processWebSocketHandshake() throws NoSuchAlgorithmException, IOException {
		...
		if (!headers.containsKey(CONNECTION_KEY) ||!headers.get(CONNECTION_KEY).contains(
				"Upgrade")) {
		...
		if (!headers.containsKey(WS_PROTOCOL_KEY) ||!headers.get(WS_PROTOCOL_KEY).contains(
				"xmpp")) {
			writeRawData(BAD_REQUEST);
			dumpHeaders(headers);
