...
public class SASLProcessor
	...
	private static final Element FEATURES = new Element("mechanisms", new Element[] {
			new Element("mechanism", "PLAIN") }, new String[] { "xmlns" }, new String[] {
	...
	public boolean process(Packet p, ComponentIOService serv,
		...
		if (p.isElement("auth", XMLNS)) {
			...
			if (auth_ok) {
				Element success = new Element("success", new String[] { "xmlns" }, new String[] {
						XMLNS });

				results.offer(Packet.packetInstance(success, null, null));
				handler.authenticated(serv);
			} else {
				Element failure = new Element("failure", new Element[] { new Element(
						"not-authorized") }, new String[] { "xmlns" }, new String[] { XMLNS });
...
//       mechanism='PLAIN'>AGp1bGlldAByMG0zMG15cjBtMzA=</auth>
	...
	private String[] decodeMessage(String input) {
		...
		result[0] = ((authoriz.length() > 0)
				? authoriz
				: null);
		result[1] = ((user_id.length() > 0)
				? user_id
				: null);
		result[2] = ((passwd.length() > 0)
				? passwd
				: null);
	...
	private String encodeMessage(String authoriz, String user_id, String password) {
		int authoriz_size = ((authoriz != null)
				? authoriz.getBytes().length
				: 0);
		int user_id_size  = ((user_id != null)
				? user_id.getBytes().length
				: 0);
		int password_size = ((password != null)
		...
		if ((password != null) && (password_size > 0)) {
			System.arraycopy(password.getBytes(), 0, result, authoriz_size + 1 + user_id_size +
					1, password_size);
		}
