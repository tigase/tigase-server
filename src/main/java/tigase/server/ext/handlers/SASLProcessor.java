...
public class SASLProcessor implements ExtProcessor {
  ...
  private static final Logger log =
    Logger.getLogger(SASLProcessor.class.getName());
	...
	private static final Element FEATURES =
			...
			new Element[]{new Element("mechanism", "PLAIN")},
			new String[]{"xmlns"}, new String[]{"urn:ietf:params:xml:ns:xmpp-sasl"});
	...
	private String encodeMessage(String authoriz, String user_id, String password) {
		...
		if (password != null && password_size > 0) {
			System.arraycopy(password.getBytes(), 0,
					result, authoriz_size+1+user_id_size+1, password_size);
		}
