	...
	public void serviceStarted(S2SIOService serv) {
		...
		case connect :
			...
			String data = "<stream:stream" +
					...
					" xmlns:db='jabber:server:dialback'" + " from='" + cid.getLocalHost() + "'" +
					" to='" + cid.getRemoteHost() + "'" + " version='1.0'>";

			if (log.isLoggable(Level.FINEST)) {
				log.log(Level.FINEST, "{0}, sending: {1}", new Object[] { serv, data });
			}

			S2SConnection s2s_conn = (S2SConnection) serv.getSessionData().get(S2SIOService
					.S2S_CONNECTION_KEY);
	...
	public String streamOpened(S2SIOService serv, Map<String, String> attribs) {
		...
		try {
			...
			case accept : {
				...
				if (cid != null) {
					stream_open += " from='" + cid.getLocalHost() + "'" + " to='" + cid
							.getRemoteHost() + "'";
					if (cid_conns == null) {
