...
//                                                 true);
	...
	private static int                      outgoingOpenTasksSize = Runtime.getRuntime()
			.availableProcessors();
	private static ScheduledExecutorService outgoingOpenTasks = Executors
			.newScheduledThreadPool(outgoingOpenTasksSize);
	...
	private AtomicBoolean                         outgoingOpenInProgress =
	 ...
	 */
	private Map<String, String>           dbKeys = new ConcurrentSkipListMap<String,
			String>();
	...
	public void reconnectionFailed(Map<String, Object> port_props) {
		S2SConnection s2s_conn = (S2SConnection) port_props.get(S2SIOService
				.S2S_CONNECTION_KEY);
		...
		outgoingOpenTasks.schedule(new Runnable() {
			...
			public void run() {
				try {
					...
					port_props.put(S2SIOService.HANDSHAKING_ONLY_KEY, S2SIOService
					...
					port_props.put(S2SIOService.HANDSHAKING_DOMAIN_KEY, verify_req.getStanzaTo()
							.toString());
	...
	public int getIncomingTLSCount() {
		...
		for (S2SConnection s2SConnection : incoming) {
			...
			if (serv.isConnected() && (serv.getSessionData().get(S2SIOService
					.CERT_CHECK_RESULT) != null)) {
				++result;
			}
		}
	...
	public S2SConnection getS2SConnectionForSessionId(String sessionId) {
		...
		for (S2SConnection s2sc : incoming) {
			if ((s2sc.getS2SIOService() != null) && sessionId.equals(s2sc.getS2SIOService()
					.getSessionId())) {
				s2s_conn = s2sc;
	...
	private void initNewConnection(String ip, int port, S2SConnection s2s_conn, Map<String,
			Object> port_props) {
		...
		if (log.isLoggable(Level.FINEST)) {
			...
			log.log(Level.FINEST, "{0} connection params: {1}", new Object[] { cid,
					port_props });
		}
	...
	private boolean openOutgoingConnections() {
		...
		try {
			...
			if (log.isLoggable(Level.FINEST)) {
				log.log(Level.FINEST, "Checking DNS for host: {0} for: {1}", new Object[] { cid
						.getRemoteHost(),
						cid });
			}
			...
			for (DNSEntry dNSEntry : dns_entries) {
				...
				for (int i = openForIP; i < max_out_conns_per_ip; i++) {
					if (dNSEntry.getIp().equals("127.0.0.1")) {
						...
						throw new UnknownHostException("DNS misconfiguration for domain: " + cid
								.getRemoteHost());
					}
	...
	private void sendPacketsBack() {
		...
		while ((p = waitingPackets.poll()) != null) {
			try {
				...
				log.log(Level.WARNING, "Packet: {0} processing exception: {1}", new Object[] { p
						.toString(),
						e });
			}
		}
