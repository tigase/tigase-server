...
public class TaskInstanceCommand
	...
	public void processCommand(Packet packet, Packet result, StanzaReceiver receiv) {
		...
		if (task != null) {
			...
			if (!(receiv.isAdmin(packet.getStanzaFrom()) || task.isAdmin(packet
					.getStanzaFrom()))) {
				Command.addFieldValue(result, "Info", "You are not administrator of: " +
						task_name + " task.", "fixed");
				Command.addFieldValue(result, "Info", "You can not execute task commands.",
						"fixed");
			...
			case USER_MANAGEMENT :
				try {
					...
					Logger.getLogger(TaskInstanceCommand.class.getName()).log(Level.SEVERE, null,
							ex);
				}
		...
		if (confirm == null) {
			...
			for (Map.Entry<String, PropertyItem> entry : old_params.entrySet()) {
				if (entry.getValue().isMultiValue()) {
					String[] values = Command.getFieldValues(packet, XMLUtils.escape(entry
							.getKey()));
					...
					if (!value.equals(entry.getValue().toString())) {
						...
						Command.addFieldValue(result, "Info", entry.getValue().getDisplay_name() +
								": " + value, "fixed");
					}
				}
	...
	private void manageUsers(Packet packet, Packet result, StanzaReceiver receiv)
		...
		if (user_action == null) {
			...
			if (moderated.size() > 0) {
				Command.addFieldValue(result, USER_ACTION_FIELD, actions[0], "Select action",
						actions, actions);
				...
				Command.addFieldValue(result, USER_ACTION_FIELD, actions[2], "Select action",
						actions, actions);
				...
				if (jids != null) {
					...
					for (String jid : jids) {
						...
						if (ri != null) {
							...
							receiv.addOutPacket(getMessage(ri.getJid(), task.getJID(), StanzaType
									.headline, "Your subscription has been approved."));
						} else {
			...
			case REJECT_PENDING_MODERATIONS :
				if (jids != null) {
					...
					for (String jid : jids) {
						...
						receiv.addOutPacket(getMessage(task.getJID(), jjids[idx++], StanzaType
								.headline, "Your subscription has been rejected."));
					}
			...
			case SELECT_USER :
				JID        jid = JID.jidInstance(Command.getFieldValue(packet,
						SUBSCRIBERS_FIELD));
				...
				if (ri != null) {
					...
					if (roster_action == null) {
						...
						Command.addFieldValue(result, "subscribed-ri", Boolean.valueOf(ri
								.isSubscribed()).toString(), "Subscribed", bool_arr, bool_arr);
						Command.addFieldValue(result, "moderation-ri", Boolean.valueOf(ri
								.isModerationAccepted()).toString(), "Moderation approved", bool_arr,
								bool_arr);
						Command.addFieldValue(result, "owner-ri", Boolean.valueOf(ri.isOwner())
								.toString(), "Owner", bool_arr, bool_arr);
						Command.addFieldValue(result, "admin-ri", Boolean.valueOf(ri.isAdmin())
								.toString(), "Admin", bool_arr, bool_arr);
					} else {
						...
						case UPDATE_DATA :
							...
							Command.addFieldValue(result, "Info", "Subscription data for user " + jid +
									" have been updated.", "fixed");

							break;

						case REMOVE_USER :
							receiv.removeTaskSubscribers(task, jid);
							Command.addFieldValue(result, "Info", "Subscription for user " + jid +
	...
	private void removeTask(Packet packet, Packet result, StanzaReceiver receiv) {
		...
		if (confirm == null) {
			...
			Command.setStatus(result, Command.Status.executing);
			Command.addAction(result, Command.Action.complete);
			Command.addFieldValue(result, "Info", "Are you sure you want to remove task: " +
					task_name + " and all it's data?", "fixed");
			Command.addFieldValue(result, "Info",
		...
		Command.addFieldValue(result, "Info", "Task " + task_name + " has been removed.",
				"fixed");
	}
	...
	private void taskMainScreen(Packet result, ReceiverTaskIfc task) {
		...
		if (stats != null) {
			for (StatRecord rec : stats) {
				Command.addFieldValue(result, "Info", rec.getDescription() + ": " + rec
						.getValue(), "fixed");
		...
		Command.addFieldValue(result, ACTION_FIELD, actions[0], "Select action", actions,
				actions);
	}
}    // TaskInstanceCommand
