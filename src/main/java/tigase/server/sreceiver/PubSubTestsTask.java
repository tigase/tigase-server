...
public class PubSubTestsTask
	...
	private static final String[] IQ_PUBSUB_SUBSCRIPTIONS_PATH = { "iq", "pubsub",
			"subscriptions" };
	private static final String TASK_HELP = "This is a PubSub component testing task." +
	...
	private Element conf = new Element("x", new Element[] { new Element("field",
			new Element[] { new Element("value",
			"http://jabber.org/protocol/pubsub#node_config") }, new String[] { "var",
			"type" }, new String[] { "FORM_TYPE", "hidden" }), new Element("field",
					new Element[] { new Element("value", "0") }, new String[] { "var" },
					new String[] { "pubsub#notify_sub_aff_state" }) }, new String[] { "xmlns",
	...
	private enum command {
		help(" - Displays help info."), newnodes(" N - Create N nodes in the pubsub."),
				...
				newnodessubscr(" N M -  create N nodes with M subscriptions each."), setdelay(
				" N - set the delay between sending node creation packet."), stop(
				" - Stops the current packets generation.");
...
//  }
	...
	private Element createPubSubEl(JID from, JID to, String id, String node,
		...
		Element elem = new Element("iq", new Element[] { new Element("pubsub",
				new Element[] { new Element(pubsub_call, new String[] { "node" }, new String[] {
				node }) }, new String[] { "xmlns" }, new String[] { xmlns }) }, new String[] {
				"type",
				"from", "to", "id" }, new String[] { "set", from.toString(), to.toString(), id });
	...
	private void runCommand(final Packet packet, Queue<Packet> results) {
		...
		case newnodes :
			...
			if (pars != null) {
				addOutPacket(Message.getMessage(packet.getStanzaTo(), packet.getStanzaFrom(),
						StanzaType.chat, "Task accepted, processing...", "PubSub testing task", null,
						packet.getStanzaId()));
				...
				results.offer(Message.getMessage(packet.getStanzaTo(), packet.getStanzaFrom(),
						StanzaType.chat, "Incorrect command parameters.", "PubSub testing task",
		...
		case newsubscr :
			if (last_nodes != null) {
				...
				if (pars != null) {
					addOutPacket(Message.getMessage(packet.getStanzaTo(), packet.getStanzaFrom(),
							StanzaType.chat, "Task accepted, processing...", "PubSub testing task",
							null, packet.getStanzaId()));
					runInThread(new Runnable() {
						@Override
						public void run() {
				...
				results.offer(Message.getMessage(packet.getStanzaTo(), packet.getStanzaFrom(),
						StanzaType.chat, "There are no pubsub nodes created yet.",
		...
		case newnodessubscr :
			...
			if (pars != null) {
				addOutPacket(Message.getMessage(packet.getStanzaTo(), packet.getStanzaFrom(),
						StanzaType.chat, "" + new Date() + " Task accepted, processing...",
						"PubSub testing task", null, packet.getStanzaId()));
				runInThread(new Runnable() {
					@Override
					public void run() {
		...
		Thread thr = new Thread() {
			...
			public void run() {
				...
				addOutPacket(Message.getMessage(packet.getStanzaTo(), packet.getStanzaFrom(),
						...
						" Generation of the test data completed.\n" + "Generated in: " + gen_hours +
						"h, " + gen_mins + "m, " + gen_secs + "sec" + ", packets generated: " +
						packetsGenerated, "PubSub testing task", null, packet.getStanzaId()));
			}
		};
